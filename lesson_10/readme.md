...
- или `with allure.step` для улучшения читаемости отчетов.
...
2. В отчете Allure вы увидите:
   1. - **`Проверка итоговой суммы покупки в интернет-магазине SauceDemo`**.
   2. - **`Проверка работы калькулятора с искусственной задержкой`**.
